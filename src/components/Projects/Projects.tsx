...
const Projects = () => {
  const [userClick, setUserClick] = useState<projectcat>(
    projectcat.Videoediting
  );
    ...
    <div className="container relative md:mt-24 mt-16" id="projects">
      <div className="grid grid-cols-1 pb-6 text-center">
        ...
        <p className="text-slate-400 dark:text-white/60 max-w-xl mx-auto">
          Dive into my freelance journey through a curated collection of
          impactful and diverse work samples.
        </p>
      </div>
            ...
            <ul className="mb-0 list-none container-filter space-x-3">
              <li
                className={`inline-block font-medium text-[15px] mb-3 text-slate-400 cursor-pointer relative duration-500 ${
                  userClick === projectcat.Videoediting ? "active" : ""
                }`}
                onClick={() => renderProject(projectcat.Videoediting)}
              >
                Video Editing
              </li>
              <li
                className={`inline-block font-medium text-[15px] mb-3 text-slate-400 cursor-pointer relative duration-500 ${
                  userClick === projectcat.thumbnail ? "active" : ""
                }`}
                onClick={() => renderProject(projectcat.thumbnail)}
              >
                Thumbnail
              </li>
              ...
              <li
                className={`inline-block font-medium text-[15px] mb-3 text-slate-400 cursor-pointer relative duration-500 ${
                  userClick === projectcat.photoediting ? "active" : ""
                }`}
                onClick={() => renderProject(projectcat.photoediting)}
              >
                Photography
              </li>
            </ul>
